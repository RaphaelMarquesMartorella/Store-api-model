  ...
  try {
    ...
    if (existingSale) {
      ...
      if (!existingRequest) {
        ...
        res.json({ error: 'There is already a request with this saleId.' }).status(404);
      }
    } else {
      res.json({ error: 'Could not find a sale that matches the saleId.' }).status(404);
    }
  } catch (error) {
    console.log(error);
...
const deleteRequest = async (req, res) => {
    try {
      ...
      console.log(error);
      res.json({ error: "It was not possible to delete this request." }).status(500);
    }
  };
